...
def load_labels_and_losses(grasp_estimator, contact_infos, global_config, train=True):
    ...
    grasp_suc_labels_pc, approach_labels_pc, print_op3 = grasp_estimator._model_func.compute_labels(tf_pos_contact_points_idx[idx], tf_pos_contact_dirs_idx[idx], tf_pos_contact_approaches_idx[idx],
                                                                                            tf_pos_finger_diffs_idx[idx], target_point_cloud, cam_poses_pl, global_config)
    print_op2 = tf.print(f'grasp success labels for idx {idx}/{scene_idx_pl}', tf.shape(grasp_suc_labels_pc), tf.reduce_mean(grasp_suc_labels_pc), output_stream=sys.stdout)
